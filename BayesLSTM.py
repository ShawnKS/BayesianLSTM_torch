...
# ATTTTTTTTTTTTTTTENTION BIAS = FALSE in this version
...
class BayesLSTM(nn.Module):
    ...
    def forward(self, x, hx):
        ...
        if is_packed:
            x, batch_sizes = x
            result = _rnn_impls['LSTM'](x, batch_sizes, hx, self._flat_weights(), True, 2, 0., self.training, False)
        else:
            result = _rnn_impls['LSTM'](x, hx, self._flat_weights(), True, 2, 0., self.training, False, False)
...
class RNNBase(Module):
    ...
    def __init__(self, mode, input_size, hidden_size,
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                w_ih_mean = Parameter(torch.Tensor(gate_size, layer_input_size))
                w_ih_logvar = Parameter(torch.Tensor(gate_size, layer_input_size))
                w_hh_mean = Parameter(torch.Tensor(gate_size, hidden_size))
                w_hh_logvar = Parameter(torch.Tensor(gate_size, hidden_size))
                b_ih_mean = Parameter(torch.Tensor(gate_size))
                b_ih_logvar = Parameter(torch.Tensor(gate_size))
                # Second bias vector included for CuDNN compatibility. Only one
                # bias vector is needed in standard definition.
                b_hh_mean = Parameter(torch.Tensor(gate_size))
                ...
                layer_params = (w_ih_mean, w_ih_logvar,
    ...
    def sample(self):
        ...
        for layer in range(self.num_layers):
            for direction in range(1):
                w_ih = self.sampled_weights[start]
                w_hh = self.sampled_weights[start+1]
                b_ih = self.sampled_weights[start+2]
                b_hh = self.sampled_weights[start+3]
                start += 4
    ...
    def __setstate__(self, d):
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                suffix = '_reverse' if direction == 1 else ''
                weights = ['weight_ih_mean_l{}{}','weight_ih_logvar_l{}{}', 'weight_hh_mean_l{}{}', 'weight_hh_logvar_l{}{}','bias_ih_mean_l{}{}',  'bias_ih_logvar_l{}{}', 'bias_hh_mean_l{}{}',  'bias_hh_logvar_l{}{}' ]
    ...
    def _flat_weights(self):
        # for i in range(5):
        #     print("-----------------------------------------------------")
        # print([layerparams for layerparams in self.all_weights])
        # print("?????????????????????????????????????????????????")
        # print(self.all_weights)
        # for i in range(5):
        #     print("-----------------------------------------------------")
        # print([p for layerparams in self.all_weights for p in layerparams])
    ...
    def all_weights(self):
        ...
        # print(self.sample()[0])
        return(self.sample())
        # print([[weight for weight in weights] for weights in self._all_weights])
        # print([[getattr(self, weight) for weight in weights] for weights in self._all_weights][0][6])
        # print("ttttttttttttttttt")
        # print("fuckfuckfuckfuckfuckfuckfuckfuckfuckfuck")
        # print([[getattr(self, weight) for weight in weights] for weights in self._all_weights][0][6].shape)

        # torch.Size([512, 61]) [0][0]
        # torch.Size([512, 128]) [0][2]
        # torch.Size([512]) [0][4]
        # torch.Size([512]) [0][6]
...
class BLSTM(RNNBase):
    ...
    def __init__(self, *args, **kwargs):
        super(BLSTM, self).__init__('LSTM', *args, **kwargs)
